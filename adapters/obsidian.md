...
- **New-note creation bypasses the taxonomy.** A stakeholder can create a top-level note in two keystrokes. Lint flags an undeclared root file. The maintainer either relocates the content through intake / `references/`, or — only if SPEC §3's ladder failed — does the complete write in the same run (entry, sync, `feeds:`, changelog ladder, `escalations:`). Intent is not a rung. Do not ask the stakeholder whether the note "earns" a canonical home.
