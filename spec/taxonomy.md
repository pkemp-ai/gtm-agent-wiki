...
- **The top level is fixed.** These files, and only these files, live at the wiki root. Depth and growth go into `references/`. A deployment may omit files that don't apply (recorded in `AGENTS.md`), but may not invent new top-level files without adding a taxonomy entry to its `AGENTS.md`.
- **One canonical home per concept.** Every boundary note below exists to prevent the same fact living in two files. When in doubt, the fact goes in the file whose *consumers* need it most, and other files link to it.
...
**Three dispositions, not two.** A canonical file that doesn't fit a deployment has three legitimate outcomes, and picking the wrong one loses content:
...
2. **File inventory table** — one row per canonical file: name, tier, `description` (pulled from front matter), `evidence-as-of`, `last-verified`. Both dates, because a consumer deciding whether to trust a file needs to know how old the evidence is *and* when a human last looked (SPEC §4.1). *Machine-generated by `scripts/sync_manifest.py`; never hand-edited.*
...
5. **Deployment notes** — omitted files and why, storage adapter in use, digest recipient and cadence.
...
A deployment may add pages, but where one of these exists it uses this name. What failed in testing was never the fan-out mechanism — it absorbed every homeless concept in all three runs — it was that nothing *named* the destination, so each page was invented once and its placement filed as a question to a stakeholder who correctly refused to answer it.
...
| `community.md` | a company-hosted community as a GTM motion: posting rules, contributor credit, triage as a marketing surface, the support answer that is actually the funnel | `channel-styles.md ## Community` |
