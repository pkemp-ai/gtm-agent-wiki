...
Synthesis reads `.archive/`, never live sources. Draft **all** canonical files in the taxonomy; a file that genuinely doesn't apply (no partner motion → no `partners.md`) is omitted and the omission recorded for `AGENTS.md` deployment notes. **Nothing goes unlabeled**: every actionable statement carries a claim tag `[<label> | <provenance> | <date>]`, with `^topic-keys` on claims agents will cite or revisit.
...
| **system** | Fill as machinery: `sources.md` (A2a/A2b), `open-questions.md` (A6), `changelog.md` (every milestone). `AGENTS.md` waits for Phase B — its summary must be ratified. | n/a |
...
**Every omission, section-level gap, and local taxonomy change gets a home the moment you make it** — the `build:draft` changelog entry, in full, and an open question wherever a human's input could still fill the gap. Don't wait for Phase B to write this down: B5 reads the changelog to generate `AGENTS.md`'s deployment notes, and the "omitted files" row can be written here in A5 — only the three-sentence company summary genuinely has to wait for ratification.

**Deviations, reasoning, and construction commentary live in the changelog, never in a canonical file's body.** A naming choice you had to make, a convention you deviated from, a paragraph explaining why a file reads the way it does — all real, all worth recording, and none of it belongs in `sources.md`, `AGENTS.md`, or any file a consumer agent reads for the business. Write it in the changelog entry instead. A marketer opening a canonical file to find where a number came from should not read three paragraphs of playbook archaeology first.
...
- **Decisions you made and are recording** — an owner obvious from the access pattern, an omission obvious from a total absence of evidence, a section placement the taxonomy's boundary notes already resolve. These are a changelog line and an `AGENTS.md` deployment-note row (A5), never a question.
...
3. **Read order** for common tasks (anything customer-facing reads `compliance-guardrails.md`, always).
4. **Reading contract** — embedded from [../consumer/AGENTS.md](../consumer/AGENTS.md).
5. **Deployment notes** — omitted files and why, local taxonomy changes, storage adapter in use, digest recipient and cadence. Pull the omissions and local-taxonomy-change list from the `build:draft` changelog entry (A5) rather than re-deriving it — that entry is the record of what A5 already decided.

### B6 · Re-lint, check conformance, deliver

1. Re-run `scripts/lint.py` and `scripts/sync_manifest.py`.
