...
Adding a source is a build-or-interview decision, not a maintain-run decision: a new input changes what the wiki can know, which files it can touch, and who is accountable for the access. Cursors, `broken:` markers, and recovery lines are the maintainer's to write on every run. If evidence clearly bears on a file outside a source's `feeds:`, widen `feeds:` in the same run and changelog it — never leave a known-false `confirmed` claim standing on a scoping technicality (SPEC §10).
